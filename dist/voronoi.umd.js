(function (global, factory) {
  ...
  (global = global || self, global.Voronoi = factory());
  ...
  var Beachsection = function Beachsection() {
    _classCallCheck(this, Beachsection);
  };
  ...
  function () {
    ...
    _createClass(Cell, [{
      ...
      value: function prepareHalfedges() {
        ...
        var edge; // get rid of unused halfedges
      ...
      value: function pointIntersection(x, y) {
  ...
  // rhill 2011-06-07: For some reasons, performance suffers significantly
  // when instanciating a literal object instead of an empty ctor
  var CircleEvent = function CircleEvent() {
    _classCallCheck(this, CircleEvent);
  ...
  // ---------------------------------------------------------------------------
  // Diagram methods
  var Diagram = function Diagram(site) {
  ...
  function () {
    ...
    _createClass(Edge, [{
      key: "setEdgeStartpoint",
      value: function setEdgeStartpoint(lSite, rSite, vertex) {
        if (!this.va && !this.vb) {
          this.va = vertex;
          this.lSite = lSite;
          this.rSite = rSite;
  ...
  function () {
    ...
    _createClass(RBTree, [{
      key: "rbInsertSuccessor",
      value: function rbInsertSuccessor(node, successor) {
        var parent;
        ...
        // to the left-most part of the tree
            ...
            node = this.getFirst(); // >>> Performance: cache previous/next nodes
      ...
      key: "rbRemoveNode",
      value: function rbRemoveNode(node) {
        // >>> rhill 2011-05-27: Performance: cache previous/next nodes
        if (node.rbNext) {
      ...
      key: "rbRotateLeft",
      value: function rbRotateLeft(node) {
        var p = node;
        var q = node.rbRight; // can't be null
      ...
      key: "rbRotateRight",
      value: function rbRotateRight(node) {
        var p = node;
        var q = node.rbLeft; // can't be null
      ...
      key: "getFirstOfTree",
      value: function getFirstOfTree(rootNode) {
        var maybeFirst = rootNode;
  ...
  function () {
    ...
    _createClass(Voronoi, [{
      ...
      key: "createVertex",
      value: function createVertex(x, y) {
      ...
      key: "createEdge",
      value: function createEdge(lSite, rSite, va, vb) {
      ...
      key: "createBorderEdge",
      value: function createBorderEdge(lSite, va, vb) {
        var edge = this.edgeJunkyard.pop();
      ...
      key: "createBeachsection",
      value: function createBeachsection(site) {
      ...
      key: "detachBeachsection",
      value: function detachBeachsection(beachsection) {
      ...
      key: "removeBeachsection",
      value: function removeBeachsection(beachsection) {
        ...
        var disappearingTransitions = [beachsection]; // remove collapsed beachsection from beachline
        ...
        for (var iArc = 1; iArc < nArcs; iArc++) {
      ...
      key: "addBeachsection",
      value: function addBeachsection(site) {
        var x = site.x;
        var directrix = site.y; // find the left and right beach sections which will surround the newly
        ...
        while (node) {
          dxl = Voronoi.leftBreakPoint(node, directrix) - x; // x lessThanWithEpsilon xl => falls somewhere before the left edge of the beachsection
          ...
          if (dxl > ε) {
            ...
            dxr = x - Voronoi.rightBreakPoint(node, directrix); // x greaterThanWithEpsilon xr => falls somewhere after the right edge of the beachsection
        ...
        if (lArc === rArc) {
          ...
          rArc.edge = newArc.edge; // check whether the left and right beach sections are collapsing
        ...
        if (lArc !== rArc) {
          ...
          var hb = bx * bx + by * by;
          var hc = cx * cx + cy * cy;
          var vertex = this.createVertex((cy * hb - by * hc) / d + ax, (bx * hc - cx * hb) / d + ay); // one transition disappear

          rArc.edge.setEdgeStartpoint(lSite, rSite, vertex); // two new transitions appear at the new vertex location
      ...
      key: "attachCircleEvent",
      value: function attachCircleEvent(arc) {
        ...
        var lSite = lArc.site;
        var cSite = arc.site;
        var rSite = rArc.site; // If site of left beachsection is same as site of
        ...
        var ay = lSite.y - by;
        var cx = rSite.x - bx;
        var cy = rSite.y - by; // If points l->c->r are clockwise, then center beach section does not
        ...
        var x = (cy * ha - ay * hc) / d;
        var y = (ax * hc - cx * ha) / d;
        var ycenter = y + by; // Important: ybottom should always be under or at sweep, so no need
      ...
      key: "detachCircleEvent",
      value: function detachCircleEvent(arc) {
      ...
      key: "connectEdge",
      value: function connectEdge(edge, bbox) {
        ...
        var fy = (ly + ry) / 2;
        var fm;
        var fb; // if we reach here, this means cells which use this edge will need
      ...
      key: "clipEdge",
      value: function clipEdge(edge, bbox) {
        var ax = edge.va.x;
        var ay = edge.va.y;
        var bx = edge.vb.x;
        var by = edge.vb.y;
      ...
      key: "clipEdges",
      value: function clipEdges(bbox) {
        // connect all dangling edges to bounding box
        // or get rid of them if it can't be done
        var edges = this.edges;
        var iEdge = edges.length;
        var edge; // iterate backward so we can splice safely
        ...
        while (iEdge--) {
          ...
          if (!this.connectEdge(edge, bbox) || !this.clipEdge(edge, bbox) || Math.abs(edge.va.x - edge.vb.x) < ε && Math.abs(edge.va.y - edge.vb.y) < ε) {
            edge.va = null;
            edge.vb = null;
      ...
      key: "closeCells",
      value: function closeCells(bbox) {
        var xl = bbox.xl;
        var xr = bbox.xr;
        var yt = bbox.yt;
        var yb = bbox.yb;
      ...
      key: "dumpBeachline",
      value: function dumpBeachline(y) {
        ...
        if (!this.beachline) {
          ...
          var bs = this.beachline.getFirst();

          while (bs) {
            log("  site %d: xl: %f, xr: %f", bs.site.voronoiId, Voronoi.leftBreakPoint(bs, y), Voronoi.rightBreakPoint(bs, y));
            bs = bs.rbNext;
          }
        }
      }
      ...
      value: function recycle(diagram) {
        if (diagram) {
      ...
      key: "compute",
      value: function compute(sites, bbox) {
      ...
      key: "leftBreakPoint",
      value: function leftBreakPoint(arc, directrix) {
        ...
        var rfocx = site.x;
        var rfocy = site.y;
        var pby2 = rfocy - directrix; // parabola in degenerate case where focus is on directrix
        ...
        var lfocx = site.x;
        var lfocy = site.y;
        var plby2 = lfocy - directrix; // parabola in degenerate case where focus is on directrix
      ...
      key: "rightBreakPoint",
      value: function rightBreakPoint(arc, directrix) {
        var rArc = arc.rbNext;
      ...
      key: "quantizeSites",
      value: function quantizeSites(sites) {
        var n = sites.length;
        var site;
